...
_ = subprocess.check_call("find {0} -name *.fasta | xargs cat  > {1}".format(file_out_fn, all_protein_f), shell=True)
...
for i in range(len(blast_contigs)):
    ...
    if e_value == 0:
        e_value = 1e-250
    e_value_log = -np.log10(e_value)
    if e_matrix[row_id][col_id] == 1:
        e_matrix[row_id][col_id] = e_value_log
    else:
        e_matrix[row_id][col_id] += e_value_log
e_weight = e_matrix-50
