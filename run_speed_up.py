...
for i in range(file_id):
    ...
    cmd = f"python 'run_construct_graph_network.py' --database_path {args.database_path} --outpath {args.outpath} --n " + str(i)
    try:
        out = subprocess.check_call(cmd, shell=True)
    except:
    ...
    cmd = f"python 'run_label_propagation.py' --database_path {args.database_path} --class_counter_path {args.class_counter_path} --outpath {args.outpath}"
    try:
        out = subprocess.check_call(cmd, shell=True)
    except:
