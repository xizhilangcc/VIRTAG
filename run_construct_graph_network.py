...
parser.add_argument('--database_path', type=str, default = "ICTV2024")
...
with open(f"{args.outpath}/name_list.csv",'w') as list_out:
    ...
    for file_n in file_list:
        for record in SeqIO.parse(contig_in+file_n, "fasta"):
            ...
            test_contig_set.add(str(record.id))
...
file_list = os.listdir(contig_out)
for file in file_list:
    for record in SeqIO.parse(file_in_fn+file, "fasta"):
        ...
        for i in range(len(proteins)):
            rec = SeqRecord(Seq(proteins[i]), id= contig_id + "_" + str(i), description="")
            protein_record.append(rec)
        _ = SeqIO.write(protein_record, file_out_fn+file, "fasta")

all_protein_f = out_f+"/test_protein.fa"
...
def make_diamond_db(cpu: int):
    diamond_db_bp = f"{folder_path}/database.dmnd"
    aa_fp = f"{folder_path}/database_with_class_prodigal_protein.fasta"
...
db_fp = f"{folder_path}/database.dmnd"
...
contig_abc_fp_bitscore = out_f + diamond_out_fn + "_bitscore.abc"
abc_fp = out_f+"merged.abc"
_ = subprocess.check_call("awk '$1!=$2 {{print $1,$2,$11}}' {0} > {1}".format(diamond_out_fp, contig_abc_fp), shell=True)
_ = subprocess.check_call("awk '$1!=$2 {{print $1,$2,$12}}' {0} > {1}".format(diamond_out_fp, contig_abc_fp_bitscore), shell=True)
_ = subprocess.check_call("cat {0}/database.self-diamond.tab.abc {1} > {2}".format(folder_path, contig_abc_fp, abc_fp), shell=True)
...
_ = subprocess.check_call("cat {0}/ALL_gene_to_genomes.csv {1}contig_gene_to_genome.csv > {2}gene_to_genome.csv".format(folder_path, out_f, out_f), shell=True)
...
ntw = create_network(matrix, singletons, thres=0, max_sig=1000)
...
# loading database
gene2genome = pd.read_csv(folder_path+"/ALL_gene_to_genomes.csv")
...
blastp = pd.read_csv(contig_abc_fp_bitscore, sep=" ", names = ["contigs", "ref", "bitscore"])
...
blast_value = blastp["bitscore"].values
for i in range(len(blast_contigs)):
    ...
    bitscore = float(blast_value[i])
    if e_matrix[row_id][col_id] == 1:
        e_matrix[row_id][col_id] = bitscore
    else:
        e_matrix[row_id][col_id] += bitscore

e_weight = e_matrix-200
...
edges_df = pd.DataFrame(columns=["node1", "node2", "weight"])
...
for i in range(e_weight.shape[0]):
    ...
    for j in range(10):
        ...
        if e_weight[i][idx] != 0:
            ...
            G.add_edge(contig_name, ref_name, weight = e_weight[i][idx])
            cnt += 1
...
with open(out_f+"/network.ntw") as file_in:
    for line in file_in.readlines():
        ...
        node1_in_contig = node1 in contig_id_set
        node2_in_contig = node2 in contig_id_set
        node1_in_graph = node1 in G.nodes()
        node2_in_graph = node2 in G.nodes()

        if node1_in_contig and node2_in_contig:
            G.add_edge(node1, node2, weight=weight)
        
        elif node1_in_contig and not node1_in_graph and not node2_in_contig:
            G.add_edge(node1, node2, weight=weight)

        elif node2_in_contig and not node2_in_graph and not node1_in_contig:
            G.add_edge(node1, node2, weight=weight)
...
if mode == "testing":
    ...
    test_data = []
    for node in G.nodes():
        if str(node) in test_contig_set:
            test_data.append(str(node))
            train_mask.append(0)
            label.append(-1)
        else:
            label_value = int(node.split('_')[2])
            train_mask.append(1)
            label.append(label_value)
    zero_indices = [index for index, value in enumerate(train_mask) if value == 0]
    test_dict = dict(zip(zero_indices, test_data))
    zero_count = train_mask.count(0)
    print("Number of zeros:", zero_count)
    pkl.dump(train_mask, open(f"{args.outpath}/Cyber_data/contig.train_mask", "wb" ) )
    ...
    pkl.dump(test_dict, open(f"{args.outpath}/Cyber_data/contig.test_dict", "wb" ) )