...
def label_propagation_accuracy(adj_matrix, labels, train_mask, test_mask, num_iterations, output_file = 'result_unknow/isolated_test_nodes_fixed_pc.txt'):
    for iteration in range(num_iterations):
        ...
        df = pd.DataFrame(list(test_predict.items()), columns=["Sequence", "Predict"])
        csv_path = "result_unknow/pred/prediction_unknow.csv"
        ...
        if iteration == 9:
            remaining_test_nodes = torch.where(test_mask == 1)[0].tolist()
            unvisited_nodes = set(remaining_test_nodes)
            connected_components = []

            while unvisited_nodes:
                current_component = []
                stack = [unvisited_nodes.pop()]
            ...
            with open(output_file, "w") as f:
                f.write("Remaining test sequences and their connected subgraph information:\n")
                for idx, component in enumerate(connected_components):
                    if len(component) >= 2:
                        component_sequences = [
                            test_dict[int(node)] for node in component if int(node) in test_dict
                        ]
                        f.write(f"Subgraph {idx + 1}: Node sequence {component_sequences}\n")
